...
xdescribe("Token contract", function () {
  ...
  async function deployFixture() {
    ...
    const OmniCogFactory = await ethers.getContractFactory("BaseReflectionBurn_aero_test", owner);
    ...
    await omniCogInstance.connect(owner).addLiquiditySolidly(
    ...
    const pairAddress = await factory.getPool(tokenList[0], tokenList[1], false);
  ...
  describe.only("Swaps", () => {
    ...
    it.only("Should reflect the correct amount on transfers (with change of LP)", async () => {
